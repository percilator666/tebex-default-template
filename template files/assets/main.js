...
/**
 * Initialize a media slider.
 * @param {HTMLElement} swiperEl
 * @param {Object} options
 * @returns {Promise<void>}
 */
function initSlider(swiperEl, options) {
    return new Promise(resolve => {
        options.on = {
            ...(options.on ?? {}),
            init: resolve
...
/**
 * Create a media lightbox for a given source slider element.
 * @param {HTMLElement} sourceContainer
 */
async function openMediaLightbox(sourceContainer, index = 0) {
    const container = document.createElement('div')
    container.classList.add('popup', 'popup-media-slider')

    const swiperEl = document.createElement('swiper-container')
    swiperEl.setAttribute('init', false)
    swiperEl.classList.add('slider')

    const
        thumbsCont = sourceContainer.querySelector('.thumbs').cloneNode(true),
        thumbs = Array.from(thumbsCont.querySelectorAll('.thumb'))

    thumbs[index].classList.add('active')

    const sourceSlides = sourceContainer.querySelectorAll('swiper-slide.slide')

    for (const slide of sourceSlides) {
        const clone = slide.cloneNode(true)

        const slideImage = clone.querySelector('img.slide-image')

        if (slideImage) {
            clone.removeChild(slideImage)

            const zoomableWrapper = document.createElement('div')
            zoomableWrapper.classList.add('swiper-zoom-container')
            zoomableWrapper.appendChild(slideImage)

            clone.appendChild(zoomableWrapper)
        }

        const youtubeFrame = clone.querySelector('.slide-frame')

        if (youtubeFrame) {
            youtubeFrame.src = youtubeFrame.src
        }

        swiperEl.append(clone)
    }
    ...
    container.append(closeButton, swiperEl, thumbsCont)
    ...
    for (const thumb of thumbs) {
        thumb.addEventListener('click', () => {
            if (swiperEl.swiper) {
                ...
                swiperEl.swiper.slideTo(index, 200)
            }
        })
    }
    ...
    const onSlideChangeTransitionEnd = () => {
        ...
        const { activeIndex } = swiperEl.swiper
    ...
    await initSlider(swiperEl, swiperParams)
}
...
function closePopup(popupContainer, pushHistory = true, remove = true) {
    ...
    if (remove) {
        const
            ...
            fadeDurationCSS = bodyStyles.getPropertyValue('--fade-duration') ?? '300ms'

        const fadeDuration = fadeDurationCSS.endsWith('ms')
            ? parseFloat(fadeDurationCSS)
            : parseFloat(fadeDurationCSS) * 1000
...
async function addToBasket(packageId, type = 'single', giftTo) {
    ...
    const popupUrl = new URL(url, location)
    popupUrl.searchParams.set('popup', '1')
...
/**
 * Initialize the slider for a given container element.
...
function setSlider(container) {
    ...
    const
        swiperEl = container.querySelector('.slider'),
        openLightboxButton = container.querySelector('.open-lightbox'),
        thumbs = Array.from(container.querySelectorAll('.thumbs .thumb'))

    openLightboxButton.addEventListener('click', () => {
        const { activeIndex } = swiperEl.swiper

        const frame = swiperEl.querySelector('iframe.slide-frame')

        if (frame) {
            frame.src = frame.src
        }

        openMediaLightbox(container, activeIndex)
    })
